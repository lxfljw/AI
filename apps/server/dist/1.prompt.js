...
function parseRagEntityLines(rag) {
    const re = /角色：([^，\n]+)，姓名：([^，\n]+)，id：(\d+)/g;
    const out = [];
    let m;
...
function parseFuzzyTargetNamesFromRag(rag) {
...
function buildVerifiedFactSystemMessage(prompt, ragForLlm) {
    ...
    if (exact) {
        ...
        return new SystemMessage(`【已核验·须遵守】参考资料中与「${exact.name}」对应的完整条目为：${line}。回答时必须包含上述全部字段（角色、姓名、id），不得只答 id、不得遗漏字段，禁止使用「参考资料中未找到」「不知道」「无法提供」。`);
    }
    if (!ragForLlm.includes("【检索说明】"))
        return null;
    ...
    const lines = hits
        ...
        .join("；");
    return new SystemMessage(`【已核验·须遵守】用户问句姓名为「${hint}」；【参考资料】已按近似匹配到资料姓名「${fuzzyNames.join("、")}」。以下条目为有效事实，必须据此完整作答（角色、姓名、id 缺一不可），并说明问句姓名与资料姓名为近似匹配：${lines}。禁止使用「参考资料中未找到」「不知道」「无法提供」。`);
}
async function main() {
    ...
    if (verified)
        messages.push(verified);
    messages.push(new HumanMessage(`【参考资料】\n${ragForLlm}\n\n【用户问题】\n${prompt}`));
...
//# sourceMappingURL=1.prompt.js.map